...
async function loginGithub() {
    ...
    try {
        ...
        setupDashboardForSource(user);
        
        const repoRes = await fetch(`https://api.github.com/users/${username}/repos?sort=updated&per_page=5`);
        const repos = await repoRes.json();
        document.getElementById('repo-list').innerHTML = repos.map(r => `<div class="list-item"><span>${r.name}</span><span style="color:var(--accent)">${r.language || 'Code'}</span></div>`).join('');
...
// === НАСТРОЙКА ИНТЕРФЕЙСА ПОД ИСТОЧНИК ===
function setupDashboardForSource(user, isMockEdu = false, transactions = []) {
    ...
    if(currentAuthSource === 'github') {
        ...
        document.getElementById('stat-1-val').innerText = '0';
        document.getElementById('stat-2-val').innerText = '0';
    ...
    drawChart();
    drawHeatmap(); // Рисуем календарь активности
}
...
function switchPage(page) {
    ...
    // Рендер тяжелых графиков при переходе на вкладку
...
function setTheme(themeName, btnElement) {
    ...
    // Синхронизируем цвета для графиков JS
    setTimeout(() => { 
        drawChart(); 
        if(currentAuthSource === '01edu') drawAuditChart(18000, 12000); 
...
// === ОТРИСОВКА ГЛАВНОГО ГРАФИКА (Линии/Столбцы) ===
function drawChart() {
    ...
    if (currentMode === 'line') {
        let pts = data.points.map((v, i) => ({
            x: px + (i * ((w - px*2) / (data.points.length > 1 ? data.points.length - 1 : 1))),
            y: (h - py) - ((v / max) * (h - py*2))
        }));

        let pathStr = `M ${pts[0].x},${pts[0].y} `;
        for (let i = 1; i < pts.length; i++) {
            let cpX = (pts[i-1].x + pts[i].x) / 2; 
            pathStr += `C ${cpX},${pts[i-1].y} ${cpX},${pts[i].y} ${pts[i].x},${pts[i].y} `;
        }

        let areaStr = pathStr + `L ${pts[pts.length-1].x},${h-py} L ${pts[0].x},${h-py} Z`;
        const area = document.createElementNS("http://www.w3.org/2000/svg", "path");
        area.setAttribute("d", areaStr); area.setAttribute("fill", "url(#areaGrad)");
        svg.appendChild(area);

        const line = document.createElementNS("http://www.w3.org/2000/svg", "path");
        line.setAttribute("d", pathStr); line.setAttribute("stroke", accentColor); 
        line.setAttribute("stroke-width", 3); line.setAttribute("fill", "none");
        line.style.filter = `drop-shadow(0px 0px 8px ${accentColor})`;
        svg.appendChild(line);

        pts.forEach((pt, i) => {
            const zone = document.createElementNS("http://www.w3.org/2000/svg", "rect");
            let segmentW = (w - px*2) / (pts.length > 1 ? pts.length - 1 : 1);
            zone.setAttribute("x", pt.x - segmentW/2); zone.setAttribute("y", 0);
            zone.setAttribute("width", segmentW); zone.setAttribute("height", h);
            zone.setAttribute("fill", "transparent"); zone.style.cursor = "crosshair";

            zone.onmouseover = () => {
                tooltip.style.opacity = 1;
                tooltip.innerHTML = `<span style="color:${accentColor};">${data.labels[i]}</span><br/>Значение: <b>${data.points[i]}</b>`;
            };
            zone.onmousemove = (e) => { tooltip.style.left = e.pageX+15+'px'; tooltip.style.top = e.pageY-30+'px'; };
            zone.onmouseout = () => tooltip.style.opacity = 0;
        ...
        data.points.forEach((v, i) => {
            ...
            rect.setAttribute("x", x); rect.setAttribute("y", h - py);
            ...
            rect.onmouseover = () => {
                rect.style.opacity = 0.7; tooltip.style.opacity = 1; 
                tooltip.innerHTML = `<span style="color:${accentColor};">${data.labels[i]}</span><br/>Значение: <b>${v}</b>`;
            };
...
// === КРУГОВОЙ ГРАФИК АУДИТОВ 01EDU ===
function drawAuditChart(up, down) {
    const svg = document.getElementById('chart-audits');
    svg.innerHTML = '';
    
    const size = 200, cx = size / 2, cy = size / 2 - 10; 
    const strokeW = 12, radiusUp = 75, radiusDown = 55;
    const maxVal = Math.max(up, down) || 1;

    const circUp = 2 * Math.PI * radiusUp;
    const circDown = 2 * Math.PI * radiusDown;
    const upFill = (up / maxVal) * circUp;
    const downFill = (down / maxVal) * circDown;

    const createRing = (r, color, maxCirc, fillAmount) => {
        const track = document.createElementNS("http://www.w3.org/2000/svg", "circle");
        track.setAttribute("cx", cx); track.setAttribute("cy", cy); track.setAttribute("r", r);
        track.setAttribute("fill", "none"); track.setAttribute("stroke", "rgba(255,255,255,0.05)"); track.setAttribute("stroke-width", strokeW);
        svg.appendChild(track);

        const circle = document.createElementNS("http://www.w3.org/2000/svg", "circle");
        circle.setAttribute("cx", cx); circle.setAttribute("cy", cy); circle.setAttribute("r", r);
        circle.setAttribute("fill", "none"); circle.setAttribute("stroke", color); circle.setAttribute("stroke-width", strokeW);
        circle.setAttribute("stroke-linecap", "round"); 
        
        circle.setAttribute("stroke-dasharray", maxCirc); circle.setAttribute("stroke-dashoffset", maxCirc);
        circle.style.transition = "stroke-dashoffset 1.5s ease-out"; circle.style.filter = `drop-shadow(0px 0px 5px ${color})`;
        circle.style.transformOrigin = "center"; circle.style.transform = "rotate(-90deg)";

        setTimeout(() => { circle.setAttribute("stroke-dashoffset", maxCirc - fillAmount); }, 100);
        svg.appendChild(circle);
    };

    createRing(radiusUp, "#00ff41", circUp, upFill);
    createRing(radiusDown, "#ff003c", circDown, downFill);
    ...
    ratioText.setAttribute("x", cx); ratioText.setAttribute("y", cy + 8);
    ratioText.setAttribute("fill", ratio >= 1 ? "#00ff41" : "#ff003c");
    ratioText.setAttribute("font-size", "26"); ratioText.setAttribute("font-weight", "900");
    ratioText.setAttribute("text-anchor", "middle"); ratioText.textContent = ratio;
    ratioText.style.filter = `drop-shadow(0px 0px 8px ${ratio >= 1 ? "#00ff41" : "#ff003c"})`;
    svg.appendChild(ratioText);

    const labelText = document.createElementNS("http://www.w3.org/2000/svg", "text");
    labelText.setAttribute("x", cx); labelText.setAttribute("y", cy + 25);
    labelText.setAttribute("fill", "var(--text-muted)"); labelText.setAttribute("font-size", "10");
    labelText.setAttribute("font-weight", "bold"); labelText.setAttribute("text-anchor", "middle"); labelText.textContent = "RATIO";
    svg.appendChild(labelText);

    const addLegend = (x, y, color, label, valStr) => {
        const dot = document.createElementNS("http://www.w3.org/2000/svg", "circle");
        dot.setAttribute("cx", x); dot.setAttribute("cy", y - 3);
        dot.setAttribute("r", 4); dot.setAttribute("fill", color); dot.style.filter = `drop-shadow(0px 0px 3px ${color})`;

        const txt = document.createElementNS("http://www.w3.org/2000/svg", "text");
        txt.setAttribute("x", x + 10); txt.setAttribute("y", y); txt.setAttribute("fill", "#fff");
        txt.setAttribute("font-size", "11"); txt.textContent = `${label} (${valStr})`;
        svg.appendChild(dot); svg.appendChild(txt);
    };

    addLegend(15, size - 10, "#00ff41", "Done", (up / 1000).toFixed(1) + "kB");
    addLegend(110, size - 10, "#ff003c", "Received", (down / 1000).toFixed(1) + "kB");
}

// === НОВОЕ: РАДАР (SKILL WEB) ===
function drawRadar() {
    ...
    // Рисуем сетку (паутину) 5 уровней
    for(let level=1; level<=5; level++) {
        ...
        poly.setAttribute("d", path + "Z"); poly.setAttribute("fill", "none"); poly.setAttribute("stroke", "rgba(255,255,255,0.1)");
    ...
    skills.forEach((s, i) => {
        ...
        const txt = document.createElementNS("http://www.w3.org/2000/svg", "text");
        txt.setAttribute("x", cx + (radius+25) * Math.cos(angle)); txt.setAttribute("y", cy + (radius+15) * Math.sin(angle));
        txt.setAttribute("fill", "var(--text-muted)"); txt.setAttribute("text-anchor", "middle"); txt.setAttribute("font-size", "12");
        txt.textContent = s.name;
        svg.appendChild(txt);
    });
    
    const dataPoly = document.createElementNS("http://www.w3.org/2000/svg", "path");
    dataPoly.setAttribute("d", valPath + "Z"); dataPoly.setAttribute("fill", color); 
    dataPoly.setAttribute("fill-opacity", "0.4"); dataPoly.setAttribute("stroke", color); dataPoly.setAttribute("stroke-width", "2");
    dataPoly.style.filter = `drop-shadow(0 0 10px ${color})`;
    svg.appendChild(dataPoly);
}

// === НОВОЕ: КРУГОВАЯ ДИАГРАММА ЯЗЫКОВ (PIE) ===
function drawPie() {
    ...
    const langs = [ {name:"Go", val: 55, col:"#00d2ff"}, {name:"JS", val: 30, col:"#f7df1e"}, {name:"Shell", val: 15, col:"#00ff41"} ];
    
    let cumulativePercent = 0;
    langs.forEach(lang => {
        const dashArray = `${lang.val} ${100 - lang.val}`;
        const dashOffset = 25 - cumulativePercent; // 25 чтобы начать сверху
        
        const circle = document.createElementNS("http://www.w3.org/2000/svg", "circle");
        circle.setAttribute("cx", 50); circle.setAttribute("cy", 50); circle.setAttribute("r", 25);
        circle.setAttribute("fill", "transparent"); circle.setAttribute("stroke", lang.col);
        ...
        legend.innerHTML += `<div style="display:flex; align-items:center; gap:5px;"><span style="display:inline-block; width:12px; height:12px; background:${lang.col}; border-radius:3px;"></span>${lang.name} (${lang.val}%)</div>`;
    });
}

// === НОВОЕ: КАЛЕНДАРЬ АКТИВНОСТИ (HEATMAP) ===
function drawHeatmap() {
    ...
    // Извлекаем RGB для прозрачности в CSS
...
const badgesList = [
    { id: 1, title: "Первая кровь", desc: "Сделать первый коммит в систему", unlocked: true, icon: "🩸" },
    { id: 2, title: "Глаз-алмаз", desc: "Провести 10 код-ревью (аудитов)", unlocked: true, icon: "👁️" },
    { id: 3, title: "Выживший", desc: "Успешно закончить Piscine", unlocked: true, icon: "🏊" },
    { id: 4, title: "Сеньор", desc: "Заработать более 100k XP", unlocked: false, icon: "👑" },
    { id: 5, title: "Ночная сова", desc: "Отправить пуш после 2:00 ночи", unlocked: false, icon: "🦉" }
];
...
// === НОВОЕ: PVP АРЕНА ===
function comparePvP() {
    const enemy = document.getElementById('pvp-enemy-login').value;
    ...
    // Имитация загрузки и рендера столбиков PvP
    res.innerHTML = `
        <div style="flex:1; text-align:center; color: var(--accent); display: flex; flex-direction: column; align-items: center;">
            <div style="font-size: 1.2rem; font-weight: bold; margin-bottom: 10px;">${myName}</div>
            <div style="height: 220px; width: 70px; background: var(--accent); box-shadow: var(--accent-glow); border-radius: 8px 8px 0 0;"></div>
            <p style="margin-top: 10px; font-weight: bold;">45k XP</p>
        </div>
        <div style="font-size: 2.5rem; font-weight: 900; margin-bottom: 50px; text-shadow: 0 0 10px rgba(255,255,255,0.3);">VS</div>
        <div style="flex:1; text-align:center; color: #b0b0b0; display: flex; flex-direction: column; align-items: center;">
            <div style="font-size: 1.2rem; font-weight: bold; margin-bottom: 10px;">${enemy}</div>
            <div style="height: 150px; width: 70px; background: rgba(255,255,255,0.2); border-radius: 8px 8px 0 0;"></div>
            <p style="margin-top: 10px; font-weight: bold;">31k XP</p>
        </div>
    `;
}
...
function exportCSV() {
    ...
    link.href = URL.createObjectURL(blob); link.download = `export.csv`;
